...
def process_case(inputs):
    ...
    mode = inputs[2]
    cond = inputs[3]
    #os.makedirs(os.path.join('searchlight_encoding_plots_confound', loc, mode), exist_ok=True)
    os.makedirs(os.path.join('searchlight_encoding_plots_ridge', loc, mode), exist_ok=True)

    ### preparing time ranges
    #time_ranges = [(t*0.1, (t+1)*0.1) for t in range(-4, 20)]
    time_ranges = [(t*0.1, (t+1)*0.1) for t in range(0, 10)]
        ...
        for sub, sub_data in tqdm(eeg_data[cond].items()):
            ...
            for start, end in time_ranges:
                ...
                for _ in range(50):
                    ...
                    ridge.fit(
                              [train_model[k] for k in train_keys],
                              [train_eeg[k] for k in train_keys],
                              )
                    preds = ridge.predict(
                              [test_model[k] for k in test_keys],
                              )
                    for k, p in zip(test_keys, preds):
                        corr = scipy.stats.spearmanr(test_eeg[k], p).statistic
                        #print(corr)
                        t_corrs.append(corr)
                    '''
    ...
    with open(os.path.join('searchlight_encoding_plots_ridge', loc, mode, '{}_{}_encoding.pkl'.format(cond, mode)), 'wb') as o:
    ...
    pyplot.savefig(os.path.join('searchlight_encoding_plots_ridge', loc, mode, '{}_{}_encoding.jpg'.format(cond, mode)), dpi=300)
...
### reading sims
models = dict()
for f in os.listdir('vecs'):
    m = f.split('.')[0]
    if m not in req:
        continue
    models[m] = dict()
    with open(os.path.join('vecs', f)) as i:
        for l in i:
            line = l.strip().split('\t')
