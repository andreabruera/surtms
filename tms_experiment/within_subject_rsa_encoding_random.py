...
def process_case(inputs):
    ...
    mode = inputs[2]
    cond = inputs[3]
    #os.makedirs(os.path.join('encoding_plots_confound', loc, mode), exist_ok=True)
    os.makedirs(os.path.join('encoding_plots_random', loc, mode), exist_ok=True)
    cond_res = dict()
    for model, model_sims in tqdm(models.items()):
        ...
        ### randomizing similarities
        model_sims = {k : v for k, v in zip(sorted(model_sims.keys()), random.sample(list(model_sims.values()), k=len(model_sims)))}
        cond_res[model] = list()
    ...
    with open(os.path.join('encoding_plots_random', loc, mode, '{}_{}_encoding.pkl'.format(cond, mode)), 'wb') as o:
    ...
    ax.set_title(cond)
    pyplot.savefig(os.path.join('encoding_plots_random', loc, mode, '{}_{}_encoding.jpg'.format(cond, mode)), dpi=300)
