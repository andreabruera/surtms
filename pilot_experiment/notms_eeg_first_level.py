...
idxs = [eeg.ch_names.index(c) for c in [
                                        #'C1', 
                                        'Cz', 
                                        #'C2', 'CP1', 'CPz', 'CP2', 'FC1', 'FCz',  'FC2'
                                        ]]
for cond, cond_data in eeg_data.items():
    collector = dict()
    for sub, sub_data in cond_data.items():
        for k, v in sub_data.items():
            if 'Er_' in k or 'Sie_' in k:
                #continue
                k = 'verbs'
            elif 'NA' not in k:
                print(k)
            ...
            for e in v:
                try:
                    collector[k].append(e)
                except KeyError:
                    collector[k] = [e]
    for k, e in collector.items():
        fig, ax = pyplot.subplots()
        ax.plot(eeg.times, numpy.average([numpy.average(i_e[idxs, :], axis=0) for i_e in e], axis=0))
        pyplot.savefig(os.path.join(out, '{}_{}.jpg'.format(k, cond)))
        pyplot.close()
            ...
            for t_i in tqdm(range(len(eeg.times))):
                for k_one_i, k_one in enumerate(sorted(sub_data.keys())): 
                    ...
                    for k_two_i, k_two in enumerate(sorted(sub_data.keys())): 
                        ...
                        one = sub_data[k_one][:, t_i]
                        two = sub_data[k_two][:, t_i]
