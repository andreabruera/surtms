...
folder = '/data/pt_03070/MorphSem_EEG_LLM/eeg-preprocessing/rawdata/EEG'
fnames = dict()
for f in os.listdir(folder):
    if '.eeg' not in f:
        continue
    subject = int(f.split('.')[0][2:])
    try:
...
for s, fs in fnames.items():
    ...
    for f in fs:
        ...
        data.drop_channels('P7')
        '''
        for elec in data.ch_names:
            if elec not in locs.keys():
                print(elec)
        with open('notms_electrode_portions.tsv', 'w') as o:
        ...
        sel_lines = list()
        for block in [1, 2, 3]:
            log_f = '{:02}_{}_b{}_{}_MorphSem2_Pilot.log'.format(
                                                                 s,
                                                                 ses,
                                                                 block,
                                                                 cond,
                                                                 )
            full_log = os.path.join(folder.replace('/EEG', '/Log'), log_f)
            print(full_log)
            #print(full_log)
